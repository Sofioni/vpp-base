...
#        fi
        ...
        TEMP_GATEWAY=$(awk -F"." '{print $1"."$2"."$3".0"}'<<<$VPP_CONTROL_ADDR)
        ip tuntap add name vhost0 mode tap
        ifconfig vhost0 $VPP_CONTROL_ADDR/24
        route del -net $TEMP_GATEWAY netmask 255.255.255.0 vhost0
...
#        fi
    ...
    if [[ -e /etc/sysconfig/network-scripts/ifcfg-${phys_int} || -e /etc/sysconfig/network-scripts/ifcfg-vhost0 ]]; then
        ...
        # dpdk vpp related change, we might not need all these network scripts for non-dpdk case as well
        if ! is_dpdk ; then
